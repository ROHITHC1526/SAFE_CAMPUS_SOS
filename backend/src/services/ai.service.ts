...
// Fake SOS Detection
export async function detectFakeSOS(userId: string, category: string): Promise<boolean> {
  ...
  if (recentIncidents >= 3) return true;

  // Check for resolved false alarms
  const falseAlarms = await prisma.incident.count({
    where: {
      studentId: userId,
      status: 'FALSE_ALARM',
  ...
  if (falseAlarms >= 2) return true;

  // Check cancelled incidents
  const cancelledRecent = await prisma.incident.count({
    where: {
      studentId: userId,
      status: 'CANCELLED',
  ...
  if (cancelledRecent >= 3) return true;

  return false;
}

// Responder Recommendation
export async function recommendResponder(
  incidentLat: number,
  incidentLng: number,
): Promise<{ userId: string; distance: number } | null> {
  const availableGuards = await prisma.securityGuard.findMany({
...
export function generateIncidentSummary(incident: any): string {
